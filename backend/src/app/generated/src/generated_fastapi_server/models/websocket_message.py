...
from generated_fastapi_server.models.websocket_message_data import WebsocketMessageData
try:
...
class WebsocketMessage(BaseModel):
    ...
    WebsocketMessage
    """ # noqa: E501
    type: StrictStr = Field(description="Type of WebSocket message")
    data: WebsocketMessageData
    timestamp: datetime = Field(description="Server timestamp when message was sent")
    __properties: ClassVar[List[str]] = ["type", "data", "timestamp"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('comment.created', 'comments.list', 'test',):
            raise ValueError("must be one of enum values ('comment.created', 'comments.list', 'test')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "data": WebsocketMessageData.from_dict(obj.get("data")) if obj.get("data") is not None else None,
